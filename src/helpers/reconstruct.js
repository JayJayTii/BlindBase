...
import { scheme } from '@/stores/SettingsStore.js'
...
export function GetInspectionMoves(cube) {
	...
	// First, try all rotations before going for a solution of 2 rotations
	// Without this, a rotation of z2 gives inspection moves of x2 y2!
	for (const rot of rotations) {
		let testCube = Object.assign(new FaceletCube(), JSON.parse(JSON.stringify(startCube)))
		testCube.Turn(rot)
		if (testCube.centers[0] == 0 && testCube.centers[1] == 1) {
			inspection.push(rot)
...
//Remove the part of a string after "//"
export function TrimComment(str) {
	const commentIndex = str.indexOf("//")
	return (commentIndex == -1) ? str : str.substring(0, commentIndex)
}

//Internally, the reconstruction is stored as numbers, these need to be converted to letters for the user.
export function ToLetters(arr, pieceType) {
	//Convert array of numbers to pairs of letters
	if (arr.length == 0)
		return ""
	return arr.map(i => scheme()[i + (pieceType == 2 ? 24 : 0)]).join('').match(/.{1,2}/g).join(' ')
}
...
export function GenerateReconBody(recon) {
	...
	summary += edgesEmpty ? "" : `\n\n//Edges\n`
	const edgePairs = ToLetters(recon.letters[1], 2).split(' ').filter(pair => pair.length > 1)
