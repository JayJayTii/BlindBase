<script setup>
	...
	watch(props.selectedCell, async () => {
		...
		cellValueInputBox.value.focus() // Prevent scrolling vertically to the input box
		window.scrollTo(x, y)
	});
	...
	const allowed_letters = computed({
		get: () => {
			const sheetType = sheetStore.getType(props.sheetID)
			if (sheetType == 1) return scheme().slice(0, 24)
			else if (sheetType == 2) return scheme().slice(24, 48)
			return speffzScheme
		}
	})
	...
	watch(cellKeyInput, (newValue, oldValue) => {
		...
		if (!allowed_letters.value.includes(inputChar.toUpperCase())) {
